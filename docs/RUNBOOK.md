...
> ⚠ **락이 막아 주지 못하는 것 — 외부 API 일일 한도** (2026-09-15 추가)
> 이 락은 *기사·알림 중복*만 막는다. 네이버 검색 API(하루 25,000회)나 OpenAI
> 한도는 **API 키 단위**라 인스턴스가 몇 개든 같은 통을 나눠 쓴다. 로컬 PC 와
> AWS worker 를 동시에 켜 두면 락 덕분에 기사는 안 겹치지만 네이버 한도는
> 두 배로 닳아 그날 남은 시간 수집이 통째로 막힌다. **안 쓰는 쪽은 반드시
> 내려야 한다.** 충돌 시 로그에 실제 락 주인 id 와 함께 이 경고가 찍힌다.

### 네이버 키워드 교대 조회 (2026-09-15)

활성 키워드가 118개까지 늘면서 매 회차(5분) 전부 조회하면 하루 33,984회로
무료 한도(25,000)를 1.4배 초과했다 — 매일 한도가 바닥나는 시점부터 그날 내내
429 로 네이버 수집이 통째로 막히고 있었다. 지금은 `select_naver_keywords()` 가
**'그룹사' 키워드(포스코 계열사명)는 매 회차 전부**, 나머지(산업·정책·통상)는
`NAVER_ROTATE_SLOTS`(기본 2) 회차로 나눠 교대 조회한다.

| | 회차당 조회 | 하루 호출 | 한도 대비 |
|---|---|---|---|
| 이전 | 118개 | 33,984회 | 136% (초과) |
| 현재 | 62~63개 | 18,000회 | 72% |

계열사 뉴스는 5분 간격 그대로고, 그 외 주제만 10분 간격이 된다. 키워드를 더
늘리면 `.env` 의 `NAVER_ROTATE_SLOTS` 를 3~4 로 올리면 된다(한 바퀴 도는 데
걸리는 시간이 늘 뿐, 빠지는 키워드는 없다 — selftest [7-3] 이 이를 검증한다).
