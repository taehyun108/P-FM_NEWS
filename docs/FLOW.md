...
- **정책·통상 주제**: ① 관심 키워드와 ② 필수 공통 키워드가 **둘 다** 매칭되고
  (한쪽이라도 비면 발송 안 함) 제목에 ✕ 제외 키워드가 없으면 → **일반 중요도 임계값을 우회**해
  발송(`priority=2`). 정책·통상 기사는 점수가 낮아 임계값을 그대로 걸면 거의 안 나가기 때문.
  단 야간 억제는 지킨다(야간엔 아침까지 대기)
