...
# ════════════════════════════════════════════════════════════════════
# 2 · 문제① — 기사량이 사람의 처리 한계를 넘어선다
# ════════════════════════════════════════════════════════════════════
# 하루 367건 · 7일 누적 2,607건은 make_decks.py 의 실측값(F, 2026-09-09 기준 7일차 무인 운영)과 동일 출처.
# 한 달 건수는 하루 평균 × 30일 환산 추정치(실측 아님 — 슬라이드에도 "환산"으로 명시).
# "사람이 건당 5분씩 확인·분석한다"는 가정 하에, 월간 소요 시간 = 월간 건수 × 5분 ÷ 60(분→시간 환산).
PER_DAY = 367
PER_WEEK = 2607
PER_MONTH = PER_DAY * 30                                          # 11,010건 (30일 환산 추정치)
MIN_PER_ARTICLE = 5
MONTH_HOURS = round(PER_MONTH * MIN_PER_ARTICLE / 60)             # 약 918시간
MONTH_FTE = round(MONTH_HOURS / 173, 1)                           # 월 소정근로 173시간 기준 환산

s = slide()
y = header(s, "01  문제① · 기사량", f"하루 {PER_DAY}건 → 한 달이면 {PER_MONTH:,}건",
           "이 많은 기사를 사람이 건당 5분씩 확인·분석한다면, 시간이 감당되지 않습니다.")

colw = (CW - Inches(0.6)) // 3
stats = [
    ("하루 수집 기사", f"{PER_DAY}건", "최근 7일 실측 평균 · 포스코·이차전지 키워드 118개 기준", NAVY),
    ("1주일만 지나도", f"{PER_WEEK:,}건", "7일 누적 수집 건수 (실측)", NAVY),
    ("한 달이면", f"{PER_MONTH:,}건", "일평균 × 30일 환산 (추정)", NAVY),
]
for i, (label, big, caption, color) in enumerate(stats):
    x = M + (colw + Inches(0.3)) * i
    box(s, x, y + Inches(0.05), colw, Inches(1.4), fill=PAPER, line=LINE, radius=0.06)
    box(s, x, y + Inches(0.05), colw, Inches(0.07), fill=color)
    text(s, x + Inches(0.24), y + Inches(0.24), colw - Inches(0.48), Inches(0.4),
         [(label, 11.5, True, INK_SOFT)], spacing=1.15)
    text(s, x + Inches(0.24), y + Inches(0.6), colw - Inches(0.48), Inches(0.5),
         [(big, 24, True, color)], spacing=1.0)
    text(s, x + Inches(0.24), y + Inches(1.06), colw - Inches(0.48), Inches(0.36),
         [(caption, 9.5, False, INK_SOFT)], spacing=1.1)

box(s, M, y + Inches(1.6), CW, Inches(0.72), fill=RGBColor(0xFB, 0xEC, 0xEA), line=RED, lw=1.0, radius=0.06)
text(s, M + Inches(0.3), y + Inches(1.78), CW - Inches(0.6), Inches(0.4),
     [("사람이 건당 5분씩 확인·분석한다면  ", 12.5, True, RED),
      (f"→ 한 달에 약 {MONTH_HOURS:,}시간 (정규직 약 {MONTH_FTE}명이 한 달 내내 매달려야 하는 양)", 12, False, INK)],
     spacing=1.2)

bullet(s, M, y + Inches(2.55), CW, [
    ("정보 과부하", "기사가 넘쳐날수록 무엇을 먼저 봐야 할지 판단하기 어렵고, 확인 기준이 사람마다 달라 일관성을 유지하기 힘듭니다"),
    ("시간이 없다", "이 많은 양을 매달 건당 5분씩만 훑어도, 정작 판단·대응에 쓸 시간이 남지 않습니다"),
], size=12.5, gap=Inches(0.66))
page(s)

# ════════════════════════════════════════════════════════════════════
# 3 · 문제② — 오보·이슈가 나오면, 골든타임이 있다
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "02  문제② · 골든타임", "잘못된 기사가 나오면, 몇 분 안에 움직여야 합니다",
           "확인이 늦어질수록, 시장은 확인되지 않은 내용만으로 먼저 반응합니다.")
bullet(s, M, y + Inches(0.1), CW, [
    ("확인되지 않은 정보도 섞여 들어온다", "오보·왜곡 보도, 노조·파업설, 경쟁사발 루머성 기사도 수백 건 속에 함께 수집됩니다"),
    ("골든타임이 있다", "정정 요청·해명자료·IR 대응은 초기 대응이 늦을수록 효과가 떨어지고, 그 사이 시장은 확인되지 않은 내용만으로 반응합니다"),
    ("사람이 놓치면 대응도 늦어진다", "수백 건 중 이 한 건을 담당자가 직접 찾아내야 하는 구조라면, 대응 시작 자체가 늦어집니다"),
], size=13, gap=Inches(0.72))
box(s, M, y + Inches(2.6), CW, Inches(0.85), fill=NAVY_SOFT, radius=0.06)
text(s, M + Inches(0.32), y + Inches(2.77), CW - Inches(0.64), Inches(0.6),
     [[("목표  ", 12.5, True, NAVY),
       ("기사 발행 → 텔레그램 알림 도달까지 5분 이내(P90).", 12, False, INK)],
      [("        ", 12.5, True, NAVY),
       ("확인은 시스템이 먼저 하고, 사람은 대응에만 시간을 씁니다.", 12, False, INK)]],
     spacing=1.3)
page(s)

# ════════════════════════════════════════════════════════════════════
# 4 · 문제③ — 봐야 할 곳이 계속 늘어난다
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "03  문제③ · 업무 범위", "배터리 산업은 계속 바뀌고, 봐야 할 곳은 많습니다",
           "대외협력은 이차전지 기술·정책, 정부 부처 동향, 환경 규제까지 동시에 봐야 합니다.")
bullet(s, M, y + Inches(0.1), CW, [
    ("이차전지·배터리", "소재·셀·전고체·나트륨 배터리 등 기술 트렌드가 몇 달 단위로 바뀝니다"),
    ("정부 부처 동향", "산업통상부·환경부 등에서 입법예고·행정예고·정책 발표가 수시로 나옵니다"),
    ("환경·통상 규제", "탄소중립, CBAM, IRA, 반덤핑 관세 등 해외 규제 변화가 사업에 직접 영향을 줍니다"),
    ("전부 한 사람 몫", "이 세 영역을 동시에, 매일 놓치지 않고 확인해야 하는 게 대외협력 업무의 특성입니다"),
], size=12.5, gap=Inches(0.62))
box(s, M, y + Inches(2.95), CW, Inches(0.62), fill=NAVY_SOFT, radius=0.06)
text(s, M + Inches(0.3), y + Inches(3.1), CW - Inches(0.6), Inches(0.4),
     [("그래서 필요한 것  ", 12.5, True, NAVY),
      ("자동 수집 + 중요도 판단 + 중요한 것만 즉시 알림", 12, False, INK)],
     spacing=1.2)
page(s)
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "04  한눈에", "기사가 여러분에게 오기까지",
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "05  화면 둘러보기 ①", "전체 기사 — 포토카드 피드",
...
text(s, M + Inches(0.34), y + Inches(2.72), CW - Inches(0.68), Inches(1.3),
     [[("팁  ", 12.5, True, NAVY),
      ...
      [("팁  ", 12.5, True, NAVY),
       ("기사 URL 을 직접 넣어 카드로 만들 수 있습니다 (하단 ‘URL 로 포토카드 만들기’).", 12, False, INK)]],
     spacing=1.4)
page(s)
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "06  화면 둘러보기 ②", "대외협력 (/ea) — 대관·통상 모니터링",
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "07  텔레그램 알림", "중요한 내용만, 골든타임 안에 도착합니다",
...
box(s, M, y + Inches(3.0), CW, Inches(1.5), fill=NAVY_SOFT, radius=0.06)
text(s, M + Inches(0.34), y + Inches(3.16), CW - Inches(0.68), Inches(1.2),
     [[("놓친 기사는 직접 등록  ", 12, True, NAVY),
       ("웹 하단 ‘URL 로 포토카드 만들기’ 또는 텔레그램 봇 DM 에 링크 전송 — 10~20초 뒤 카드가 생깁니다.", 11.5, False, INK)],
      [("직접 등록한 기사  ", 12, True, NAVY),
       ("임계값을 넘으면 채널에도 자동 발송됩니다 (사람이 고른 것이라 야간·억제 무시).", 11.5, False, INK)],
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "08  중요도 점수", "0~100점, 규칙으로 매깁니다",
...
# ════════════════════════════════════════════════════════════════════
s = slide()
y = header(s, "09  마스터 설정", "내가 원하는 알림만 받기",
...
items = [
    ...
    ("3. 즐겨찾기·필터 익히기", "중요도순 정렬 + 관심 카테고리로 나만의 화면 구성, 주간동향에서 한 주 요약 확인"),
