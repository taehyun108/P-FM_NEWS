...
def build_practitioner(template: str, out: str) -> None:
    ...
    agenda(prs, [
        ("지금 우리는 이렇게 한다", "손이 가장 많이 가던 일", "2분"),
        ("무엇을 만들었나", "5분마다 도는 파이프라인 8단계", "2분"),
        ("어떻게 쓰나", "웹 · 대외협력 · 텔레그램 세 화면", "4분"),
        ("무엇이 달라졌나", "7일 무인 운영 실측", "2분"),
    ])

    # ── 01 문제 ───────────────────────────────────────────────────
    slide_section(prs, "01", "지금 우리는 이렇게 한다",
                  "대외협력 담당자가 매일 반복하던 네 가지")
    s = slide_plain(prs, "손이 가장 많이 가던 일",
                    "포털·언론사를 사람이 직접 돌면서 생기는 문제", "0:00 – 2:00")
    check_rows(s, [
        ("확인 주기가 사람마다 다르다",
         "아침에 한 번 보는 사람, 수시로 보는 사람 — 놓치는 기사가 사람마다 다르다"),
        ("놓치면 대응이 늦는다",
         "특히 규제·지역 이슈는 초동 대응 시점이 곧 결과다"),
        ("과거 기사를 다시 못 찾는다",
         "검색을 반복해야 하고 링크는 만료된다"),
        ("읽어야 할 기사와 아닌 기사를 가르는 데 시간이 든다",
         f"통신사 전재로 같은 기사가 10~20건씩 들어온다 (언론사 {F['press']}곳 모니터링)"),
    ], y=BODY_TOP + 0.3, h=1.7, gap=0.42)

    # ── 02 해결 ───────────────────────────────────────────────────
    slide_section(prs, "02", "무엇을 만들었나",
                  "사람은 '확인'이 아니라 '판단'에만 시간을 쓴다")
    s = slide_plain(prs, "기사 하나가 거치는 길",
                    "5분마다 자동으로 도는 8단계 — 사람 손이 닿는 곳은 마지막 판단뿐",
                    "2:00 – 4:00")
    flow_steps(s, [
        ("수집", f"구글뉴스 RSS · 네이버 검색에서 키워드 {F['keywords']}개로 최신 기사 목록을 받는다"),
        ("게이트 7단계", "이미 본 기사 · 오래된 기사 · 중복 기사 · 포스코와 무관한 기사를 여기서 버린다"),
        ("AI 분석 1회", "요약 · 포스코 관점 · 키워드 · 감성 · SWOT 을 한 번에 뽑는다 (기사당 1회)"),
        ("중요도 채점", "퓨처엠 제목 언급 +50, 계열사 +25, 정책·통상 가점, 단순 시황 감점 → 0~100"),
        ("저장", "웹에는 전부 올라간다. 원문 본문은 요약용으로만 임시 보관 후 삭제"),
        ("알림 판정", "억제 아님 · 6시간 이내 · 주제 토글 · 중요도 ≥ 임계값 — 넷을 다 만족해야 큐로"),
        ("채널 게시", "요약 · 관점 · 링크가 담긴 개별 카드. 3.5초 간격으로 텔레그램 한도 준수"),
        ("주간 레포트", "매주 월요일 07시, 한 주치를 그룹사 5 + 정책·통상 2 섹션으로 이메일 발송"),
    ], y=BODY_TOP + 0.05, h=1.12)
    ...
    s = slide_plain(prs, "③ 텔레그램 — 봐야 할 것만 온다",
    ...
    check_rows(s, [
        ...
        ("밤에는 조용하다",
         "23~07시에는 중요도 80 이상만 즉시. 나머지는 아침에 나간다"),
        ("기준은 마스터 패널에서 바꾼다",
    ...
    s = slide_plain(prs, "7일간의 기록", "사람이 개입한 시간 0분", "8:00 – 10:00")
