...
def send_notifications(ctx: Context, limit: int = 20) -> int:
    ...
    if use_digest:
        ...
        for row in normal:
            ...
            body.append(f'{mark} <a href="{esc_attr(link)}">{esc(row.get("title") or "")}</a>')
        ok, err = _telegram_send(ctx, url, clamp_message("\n".join(body)))
        for row in normal:
            if ok:
                ctx.storage.mark_notification(row["id"], "sent", None)
            else:
                # 개별 발송과 같은 규칙으로 포기 시점을 둔다. 예전에는 실패해도 늘
                # 'queued' 로만 적어서, retry_count 가 3을 넘으면 pending 조회에서
                # 빠지는데 status 는 queued 인 채로 남았다 — 재시도도 안 되고
                # '발송 실패' 로 세어지지도 않는, 화면 어디에도 안 보이는 상태였다.
                status = "queued" if int(row.get("retry_count") or 0) < 2 else "failed"
                ctx.storage.mark_notification(row["id"], status, err)
