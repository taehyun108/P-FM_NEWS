...
create table if not exists run_state (
  ...
  notify_paused    INTEGER not null default 0,   -- 텔레그램 발송 중지 (마스터 토글·/stop). 1=중지
  notify_threshold INTEGER,                -- /threshold 로 지정한 임계값 (null 이면 .env 값)
  hard_notify_score INTEGER,               -- 무조건 발송 점수. 이 값 이상이면 야간·묶음 우회 (null·0=미사용)
  tg_offset        INTEGER not null default 0,   -- 텔레그램 getUpdates offset
  always_notify_keywords TEXT default '[]', -- 이 단어가 제목·계열사에 있으면 임계값 무관 무조건 알림
  exclude_notify_keywords TEXT default '[]', -- 이 단어가 제목에 있으면 임계값을 넘어도 알림 안 함 (최우선)
  always_kw_bypass_night INTEGER not null default 1, -- '무조건 받을 키워드' 기사를 야간에도 즉시 발송 (마스터 체크)
  policy_exclude_keywords TEXT default '[]', -- 정책 주제: 제목에 있으면 이 주제 알림 제외
  trade_exclude_keywords TEXT default '[]',  -- 통상 주제: 제목에 있으면 이 주제 알림 제외
  ...
  pipeline_lock_at TEXT,                    -- 위 락을 마지막으로 갱신한 시각
